...
describe Airport do
  ...
  let (:flying_plane)    { double :flying_plane, status: :flying, take_off_from: nil  }
  let (:landed_plane)    { double :landed_plane, status: :landed                      }

  def make_good_weather
    allow(airport).to receive(:is_weather_sunny?).and_return(true)
  end

  def make_bad_weather
    allow(airport).to receive(:is_weather_sunny?).and_return(false)
  end
